...
i386_vm_init(void)
{
	...
	//n = ROUNDUP(sizeof (struct Runqueue), PGSIZE);
	//rq = boot_alloc(n, PGSIZE);
//	if(rq == NULL)
...
//		panic("FAILED TO ALLOCATE ENVS");
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	//boot_map_segment(pgdir,KERNBASE,~KERNBASE+1,0,PTE_P | PTE_W);
...
page_init(void)
{
	...
	// mark the IO hole as used
	for (i = IOPHYSMEM; i < EXTPHYSMEM; i += PGSIZE) {  
		ppage = pa2page(i);  
		LIST_REMOVE(ppage, pp_link);  
	}  

	// mark the kernel data structure as used.
...
{
	if(pp->pp_ref !=0)
		panic("FREEING A PAGE WHEN ref != 0");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	struct Page *page;
	pde_p=&pgdir[PDX(va)];
	if(!(*pde_p & PTE_P)) { // the page present bit is 0
	// check create value
		if(!create)
			return NULL;
		if(page_alloc(&page))
		...
		//Map the linear address to this page, with present bit.
		//TODO: what about write bit and user bit?
		*pde_p= page2pa(page) | PTE_P | PTE_W | PTE_U;
	}
	// convert the page address pde_p to virtual address,
	// but first, we need the page it is in.
	pte_p = (pte_t*) KADDR(PTE_ADDR(*pde_p));
	// after getting the physical address, continue translation
	return &pte_p[PTX(va)];
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm) 
{
	...
	if(pte_p==NULL)
		return -E_NO_MEM;
	if(*pte_p & PTE_P) { //There is already a page mapped at 'va'
		if(page2pa(pp) == PTE_ADDR(*pte_p)) { // pp is re-inserted @ the same va
...
boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	for(i=0;i<size; i+=PGSIZE) {
		pte_p=pgdir_walk(pgdir,(void*)(la+i),1);
		if(pte_p==NULL)
			panic("FAILED WHILE MAPPING SEGMENT");
...
page_remove(pde_t *pgdir, void *va)
{
	struct Page* page;
	pte_t* pte_p;
	page=page_lookup(pgdir,va,&pte_p);
	if(page==NULL)
		return;
	page_decref(page);
	*pte_p = 0;
...
/* Challenge */
int 	
is_page_allocated(struct Page* pp)
{
	struct Page* pp0;
	LIST_FOREACH(pp0, &page_free_list, pp_link) {
		if(pp0==pp) {
