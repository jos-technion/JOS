...
env_setup_vm(struct Env *e)
{
	...
	// Inserting pg_dir into itself O.o
	e->env_pgdir[PDX(VPT)] = PADDR(e->env_pgdir)|PTE_W|PTE_P;
	e->env_pgdir[PDX(UVPT)] = PADDR(e->env_pgdir)|PTE_U|PTE_P;
	// Mapping Pages
	memset(e->env_pgdir, 0, PGSIZE); 
	size = (NPDENTRIES - PDX(UTOP)) * sizeof(pde_t);
	// Copy everything above UTOP
	memmove(&e->env_pgdir[PDX(UTOP)], &boot_pgdir[PDX(UTOP)], size);
	// Incrementing env_pgdir's pp_ref
	p->pp_ref++;
...
segment_alloc(struct Env *e, void *va, size_t len)
{
	...
	while(va < va_end) {
		...
		if((r = page_insert(e->env_pgdir, page , va, PTE_W | PTE_U )) != 0)
			panic("segment_alloc: %e", r);
		va += PGSIZE;
	}
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	for (; ph < eph; ph++) {
		if(ph->p_type == ELF_PROG_LOAD) {
			...
			// Should use this environment cr3
			lcr3(e->env_cr3);
...
env_run(struct Env *e)
{
	...
	//{
		...
		e->env_runs++;
		lcr3(e->env_cr3);
		//TODO: should we always change the trape frame or on context switch only?
		env_pop_tf(&e->env_tf);
	//}
}
