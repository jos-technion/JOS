	   ...
	   void **pg_store, int *perm_store)
{
	...
	// Open the file
	if (req->req_omode & O_CREAT || req->req_omode & O_MKDIR) {
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	if((r = file_read(o->o_file, (void*)ret->ret_buf, bytes, o->o_fd->fd_offset)) >=0) 
		o->o_fd->fd_offset += r;
	return r;
}
...
int
serve_mkdir(envid_t envid, struct Fsreq_create *req) {
	struct File *f;
	
	return file_create(req->req_path,&f);
}
